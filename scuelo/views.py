...
from django.views.generic import DetailView , ListView  ,TemplateView, ListView, CreateView, UpdateView, DeleteView
from django.db.models import Count, Sum , Prefetch
from django.db.models import Q
from .forms import InscriptionForm , InscriptionFormSet , EleveCreateForm ,  EleveUpdateForm , PaiementForm
...
class StudentUpdateView(UpdateView):
    ...
    def get_success_url(self):
        return reverse_lazy('student_detail_in_per_classe', kwargs={'pk': self.object.pk, 'class_id': self.object.classe.id})
...
class PaiementCreateView(CreateView):
    ...
    template_name = 'scuelo/paiment/create.html'
    success_url = reverse_lazy('success_message')'''
    

class ManagePaymentsView(FormView):
    template_name =  'scuelo/paiment/manage.html'
    form_class = PaiementForm
    success_url = '/manage/payments/'  # Adjust the URL as needed

    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        InscriptionFormSet = inlineformset_factory(Paiement, Inscription, form=InscriptionForm, extra=1)
        context['formset'] = InscriptionFormSet()
        return context
    ...
    def form_valid(self, form):
        ...
        if formset.is_valid():
            paiement = form.save()
            formset.instance = paiement
            formset.save()
            return super().form_valid(form)
        else:
            return self.render_to_response(self.get_context_data(form=form, formset=formset))

class ManagePaymentsView(TemplateView):
    template_name = 'scuelo/paiment/manage.html'
...
class PaiementCreateView(CreateView):
    ...
    form_class = PaiementForm
    template_name = 'scuelo/paiment/manage.html'
    success_url = reverse_lazy('manage_payments')
